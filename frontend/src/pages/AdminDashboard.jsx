...
const AdminDashboard = () => {
  ...
  const createService = async (e) => {
    ...
    try {
      ...
      toast.error('Failed to create service.', { theme: 'colored' });
    }
  };
  ...
  const deleteService = async (serviceId) => {
    try {
      ...
      toast.error('Failed to delete service.', { theme: 'colored' });
    }
  };
  ...
  const updateApplicationStatus = async (applicationId, status) => {
    try {
      ...
      toast.success(`Application status updated to ${status}!`, { theme: 'colored' });
    } catch (error) {
      console.error('Error updating status', error);
      toast.error('Failed to update application status.', { theme: 'colored' });
    }
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {isLoadingApplications
            ...
            : applications.map((application) => (
              <div
                key={application._id}
                className={`p-4 rounded-lg shadow-md border border-zinc-600 bg-zinc-700`}
              >
                <p className="text-zinc-300">
                  <strong>User:</strong> {application.user.name}
                </p>
                <p className="text-zinc-300 mt-1">
                  <strong>Service:</strong> {application.service.title}
                </p>
                <p className="text-zinc-300 mt-1">
                ...
                <div className="flex justify-between mt-4">
                  <button
                    ...
                    className="py-1 px-2 bg-green-600 rounded-md text-white hover:bg-green-500 transition-all"
                  >
                    Approve
                  </button>
                  <button
                    onClick={() => updateApplicationStatus(application._id, 'Rejected')}
                    className="py-1 px-2 bg-red-600 rounded-md text-white hover:bg-red-500 transition-all"
                  >
                    Reject
                  </button>
                </div>
